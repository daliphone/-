...
# --- 1. 頁面配置與 UI (遵循 v14.3.9 清新風格) ---
st.set_page_config(page_title="馬尼通訊 營銷發想系統 v14.4.1", page_icon="🐎", layout="centered")
...
DEFAULT_TIPS = {
    ...
    "step2_bait": "實戰建議：馬年慶利用『紅包感』降低支付痛苦，提升參與率。",
    "step3_path": "破冰話術：『這張抽獎券是送您的，要不要試試手氣？』引導回訪機制是關鍵。",
    "step6_metrics": "成效檢核：務必包含『數據資產累積』，例如蒐集到的問卷數量與 LINE 增加人數。"
}

if 'logic_state' not in st.session_state: st.session_state.logic_state = {m[0]: m[2] for m in MODULES}
...
for fid, title, guide in MODULES:
    ...
    # 按鈕水平對齊 (遵循 v14.3.9)
    c_ai, c_tip = st.columns([1, 2])
    with c_ai:
        st.markdown('<div class="ai-btn-small">', unsafe_allow_html=True)
        if st.button(f"🪄 AI 優化檢核", key=f"btn_{fid}"):
            if fid == "step6_metrics":
                # 專屬第六步 AI 檢核邏輯
                st.session_state[fid] = f"【AI 成效檢核診斷】：\n1. 漏斗完整性：是否包含進店量、轉化率指標？\n2. 數據資產：是否記錄 LINE 好友增加目標？\n3. 質化指標：是否設計了消費者反饋問卷？\n---\n原始內容：\n{st.session_state[fid]}"
            else:
                st.session_state[fid] = f"【AI 營銷建議】針對{title}的強化：\n{st.session_state[fid]}"
            st.rerun()
...
def generate_word():
    doc = Document()
    doc.add_heading(f'馬尼通訊 營銷執行提案書 v14.4.1', 0)
