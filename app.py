...
# --- 1. 頁面配置 ---
st.set_page_config(page_title="馬尼通訊 企劃排程系統 v14.1.4", page_icon="🐎", layout="wide")
...
if 'templates_store' not in st.session_state:
    st.session_state.templates_store = {
        "🐎 馬年慶：百倍奉還": {
            ...
            "marketing": "FB/IG/Threads 倒數限時動態；針對弱勢分店進行 3-5 公里區域廣告投遞。",
            "risk": "稅務申報(>$1000)；序號防偽蓋章；銷售分佈不均調度機制。",
...
with c1:
    ...
    st.text_area("三、 活動時程安排 (執行重點建議)", key="p_schedule", height=120, placeholder="日期與細節...")
...
with c2:
    ...
    st.text_area("七、 風險管理與注意事項 (規範建議)", key="p_risk", height=100, placeholder="稅務、序號防偽等...")
    st.text_area("八、 預估成效 (效益面建議)", key="p_effect", height=100, placeholder="觸及人次與轉化成效...")
